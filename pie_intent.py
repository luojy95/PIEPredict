...
# K.set_image_dim_ordering('tf') # in keras 2.2.5, change to the next line
K.tensorflow_backend.set_image_dim_ordering('tf')

os.environ['PIE_PATH']='./PIE_dataset'
os.environ['PIE_RAW_PATH']='./PIE_dataset/PIE_clips'
...
class PIEIntent(object):
    ...
    def load_images_and_process(self,
        ...
        print('\n\n\n', "img_sequences", len(img_sequences), len(img_sequences[0]), len(ped_ids), '\n\n\n')
    ...
    def get_test_data(self, data, train_params, seq_length, overlap):
        ...
        test_img = self.load_images_and_process(images,
                                                ...
                                                save_path=self.get_path(type_save='data',
                                                                        ...
                                                                        data_subset='test'))
        output = output[:, 0]
        print('\n\n\n', "JJJJJJJ", test_img.shape, '\n\n\n')
        print('\n\n\n', "LLLLLLL", decoder_input[0], '\n\n\n')
    ...
    def get_test_data_gem(self, data, train_params, seq_length, overlap):
        """
        A helper function for test data generation that preprocesses the images, combines
        different representations required as inputs to encoder and decoder, as well as
        ground truth and returns them as a unified list representation.
        ...
        test_img = self.load_images_and_process_gem(images,
                                                bboxes,
                                                ped_ids,
                                                data_type='test',
                                                save_path=self.get_path(type_save='data',
    ...
    def load_images_and_process_gem(self,
                                img_sequences,
                                bbox_sequences,
                                ped_ids,
                                save_path,
        ...
        #img_sequences = [1661, 15] nested list
        print('\n\n\n', "img_sequences", len(img_sequences), len(img_sequences[0]), len(ped_ids), '\n\n\n')
        sequences = []
        i = -1
        for seq, pid in zip(img_sequences, ped_ids):
            ...
            for imp, b, p in zip(seq, bbox_sequences[i], pid):
                ...
                if os.path.exists(img_save_path) and not regen_pkl:
                    ...
                    with open(img_save_path, 'wb') as fid:
                        pickle.dump(img_features, fid, pickle.HIGHEST_PROTOCOL)
                img_features = np.squeeze(img_features)
                img_seq.append(img_features) # img_features 7,7,512

            sequences.append(img_seq) # img_seq 15,7,7,512
        sequences = np.array(sequences)
        return sequences # 1661,15,7,7,512

    def load_pie_intent(self, model_path='data/pie/intention/context_loc_pretrained'):
        # Load PIE model
    ...
    # test GEM
    def test_gem_v0(self, img_sequence, bbox_sequence, model_path='data/pie/intention/context_loc_pretrained'):
            ...
            # preprocessing
            img_seq = []
            for img_data, b in zip(img_sequence, bbox_sequence):
                bbox = jitter_bbox_gem(img_data, [b],'enlarge', 2)[0]
                ...
                img_seq.append(img_features) # (15,7,7,512)


            sequences = np.array([img_seq])
            bbox_sequences = np.expand_dims(np.array(bbox_sequence), axis=0)

            test_data_chunk = [sequences, bbox_sequences]

            end = time.time()
            print("Crop + VGG: {}s".format(end - start))

            start = time.time()
            # test_data_chunk [(N, 15, 7, 7, 512), (N, 15, 4)] # [feature, bbox]
            #print('\n\n\n', test_data_chunk, '\n\n\n')
            test_results_chunk = self.pie_model.predict(test_data_chunk,
                                                    batch_size=self.train_params['batch_size'],
                                                    verbose=1)
            end = time.time()
            print("PIE intent: {}s".format(end - start))
            return test_results_chunk

    # test GEM
    def test_gem_v1(self, feature_sequence, bbox_sequence, model_path='data/pie/intention/context_loc_pretrained'):
            sequences = np.array([feature_sequence])
            bbox_sequences = np.expand_dims(np.array(bbox_sequence), axis=0)
    ...
    def test_chunk(self,
            ...
            for i in range(0, len(data_test['image']), 100):
                ...
                test_data_chunk, test_target_data_chunk = self.get_test_data(data_test_chunk, train_params, self._sequence_length, overlap)



                tracks, images_chunk, bboxes_chunk, ped_ids_chunk = self.get_tracks(data_test_chunk, train_params['data_type'], self._sequence_length, overlap)
